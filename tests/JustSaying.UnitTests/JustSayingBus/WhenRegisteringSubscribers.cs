...
public sealed class WhenRegisteringSubscribers(ITestOutputHelper outputHelper) : GivenAServiceBus(outputHelper), IDisposable
{
    ...
    protected override async Task WhenAsync()
    {
        ...
        var messageConverter1 = new ReceivedMessageConverter(new NewtonsoftMessageBodySerializer<OrderAccepted>(), new MessageCompressionRegistry(), false);
        var messageConverter2 = new ReceivedMessageConverter(new NewtonsoftMessageBodySerializer<OrderRejected>(), new MessageCompressionRegistry(), false);
