...
{
    public class WhenRegisteringASqsTopicSubscriberWithPrecachedTopicsEnabledByDefault : TopicPreloadTestBase
    {
        ...
        [Then]
        public void TopicsWereAddedToRegionalTopicCache()
        {
            int itemsAddedToCache = ANumberLargerThanAWSPageSize + 1;
            Assert.That(_queueCache.Count,Is.EqualTo(1));
            Assert.That(_queueCache.First().Value.Count, Is.GreaterThanOrEqualTo(itemsAddedToCache));
        }

        [Then]
        public void CachedTopicsHaveCorrectName()
        {
            ...
            Assert.That(cacheEntry.Arn.EndsWith(":" + testHandlerTopicName));
        }
